...
import { HiUpload, HiOutlineDocumentAdd, HiArrowLeft } from "react-icons/hi";
import { motion } from "framer-motion";
import ReactQuill from "react-quill-new";
import "react-quill-new/dist/quill.snow.css";
import { createBlog } from "../utilities/blog";
...
const CreateBlog = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!title || !slug || !category || !author || !content || !cover_image) {
      ...
      const response = await createBlog(blogPayload, cover_image);

      if (response.success) {
        SweetAlert.alert(
          "Success",
          response.message || "Blog published successfully!",
          "success"
        );
        ...
        SweetAlert.alert(
          "Error",
          response.message || "Failed to publish blog.",
          "error"
        );
      }
    } catch (error) {
    ...
    >
      <Card className="w-full shadow-md border rounded-2xl md:p-6 bg-white">
        ...
        <div className="flex items-center gap-2 mb-6">
          <HiOutlineDocumentAdd className="text-primary-600 text-3xl" />
